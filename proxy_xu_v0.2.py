...
class Proxy(threading.Thread):
    ...
    def _process_response(self, data,flag):
       ...
       # print("sa:", self.server.addr)
        if not flag:
            if self.request.build_url() not in buffer:# and self.request.method != b'CONNECT':
                print("creating buffer")
                print("a:", self.server.addr)
                buffer[self.request.build_url()] = data
            elif self.request.build_url() in buffer:# and self.request.method != b'CONNECT':
                print("adding buffer")
                print("a:", self.server.addr)
                buffer[self.request.build_url()] += data
    ...
    def _process_rlist(self, r):
        ...
        if self.server and not self.server.closed and self.server.conn in r:
            ...
            if self.first_time:
                ...
                if self.request.build_url() in buffer: #and self.request.method != b'CONNECT':
                    print("using buffer:", self.request.build_url())
                    print('first time + already in')
                    self.Read=True
                    data=buffer[self.request.build_url()]
                    flag=True
                else:
                ...
                if self.Read:
                    ...
                    if self.request.build_url() in buffer:
                        print('but ',self.request.build_url(),' in buffer')
                    else:
                        print('and ',self.request.build_url(),' not in buffer')
                    data = self.server.recv()
                    flag = False
            self.last_activity = self._now()
            if not data:
